...
type Role struct {
    ...
    // PolicyName is the policy the created user will have on the
    // Minio server
    ...
    // StsMaxTTL is the maximum TTL that STS token can exist before it expires for this role
    StsMaxTTL time.Duration `json:"sts_max_ttl"`
}
